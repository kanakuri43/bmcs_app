...
namespace bmcs_app.Order.Services;

/// <summary>
/// 受注登録用マスタ検索サービス。
/// 起動時に読み込んだ得意先・商品リストを保持し、ダイアログ表示とコード補完を提供する。
/// </summary>
public class LookupService
{
    ...
    public void Initialize(IEnumerable<Customer> customers, IEnumerable<Employee> employees, IEnumerable<Product> products)
    {
    ...
    public Customer? OpenCustomerSearch(string initialKeyword = "")
    {
        ...
        return dlg.ShowDialog() == true ? (Customer)dlg.SelectedSearchItem!.Source : null;
    }
    ...
    {
        var dlg = new SlipSearchDialog("受注検索", _slipColumns, _slipRows, keyColumnIndex: 1, initialKeyword)
            { Owner = Application.Current.MainWindow };
