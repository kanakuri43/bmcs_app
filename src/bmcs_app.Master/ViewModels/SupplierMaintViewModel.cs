...
public class SupplierMaintViewModel : BindableBase
{
    ...
    public int TotalPages => _filteredSuppliers.Count == 0
        ? 1
        : (int)Math.Ceiling(_filteredSuppliers.Count / (double)PageSize);

    public string PageLabel  => $"{CurrentPage} / {TotalPages} ページ";
    public string RangeLabel => _filteredSuppliers.Count == 0
        ? "0 件"
        : $"{(CurrentPage - 1) * PageSize + 1}〜{Math.Min(CurrentPage * PageSize, _filteredSuppliers.Count)} 件 / 全 {_filteredSuppliers.Count} 件";
    ...
    public Supplier? SelectedSupplier
    {
        ...
        set
        {
            if (SetProperty(ref _selectedSupplier, value) && value is not null)
                LoadToForm(value);
        }
    }
    ...
    // ── コンストラクタ ─────────────────────────────────────
    public SupplierMaintViewModel(ISupplierRepository repo)
    {
        _repo = repo;
        ...
        DeleteCommand = new DelegateCommand(async () => await OnDeleteAsync(), CanDelete)
                            .ObservesProperty(() => SelectedSupplier);
    ...
    private void ApplyFilter()
    {
        ...
        _filteredSuppliers = string.IsNullOrEmpty(kw)
            ? _allSuppliers
            : _allSuppliers.Where(s =>
                s.SupplierCode.Contains(kw, StringComparison.OrdinalIgnoreCase) ||
