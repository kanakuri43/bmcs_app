...
public class OrderRepository : IOrderRepository
{
    ...
    /// <summary>
    /// 伝票検索ダイアログ用。受注明細を非正規化した全行を返す（直接 SQL）。
    /// 列順: 受注日付, 受注No., 得意先コード, 得意先名, 行番号, 商品コード, 商品名, 数量, 単価, 金額
    /// </summary>
    public async Task<IEnumerable<string[]>> GetAllFlatAsync()
    {
        ...
        cmd.CommandText = @"
            SELECT o.order_date, o.order_no,
                   o.customer_code, o.customer_name,
                   o.line_no, o.product_code, o.product_name,
                   o.quantity, o.unit_price,
                   o.quantity * o.unit_price AS line_amount
            FROM   orders o
            WHERE  o.is_deleted = 0
            ORDER  BY o.order_date DESC, o.order_no, o.line_no";
    ...
    public async Task<IEnumerable<SlipSummary>> GetSummariesAsync()
    {
        ...
        await conn.OpenAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "usp_orders_summaries_select";
    ...
    public async Task<OrderSlip?> GetByOrderNoAsync(string orderNo)
    {
        OrderSlip? slip = null;
        await using var conn = new SqlConnection(ConnectionString);
        await conn.OpenAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "usp_orders_select";
        cmd.CommandType = CommandType.StoredProcedure;
        ...
        while (await reader.ReadAsync())
        {
            if (slip is null)
            {
                slip = new OrderSlip
                {
                    ...
                    HasSales      = reader.GetBoolean(reader.GetOrdinal("has_sales")),
                };
            }

            slip.Lines.Add(new OrderLine
            {
    ...
    public async Task UpsertAsync(
        string orderNo, DateOnly orderDate, int customerId, int employeeId,
        string? slipRemarks, IEnumerable<OrderLineInput> lines)
    {
        ...
        await conn.OpenAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "usp_orders_upsert";
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Parameters.AddWithValue("@order_no",     orderNo);
        cmd.Parameters.AddWithValue("@order_date",   orderDate.ToDateTime(TimeOnly.MinValue));
    ...
    public async Task DeleteAsync(string orderNo)
    {
        ...
        await conn.OpenAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "usp_orders_delete";
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Parameters.AddWithValue("@order_no", orderNo);
        await cmd.ExecuteNonQueryAsync();
    }
