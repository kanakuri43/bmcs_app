CREATE OR ALTER PROCEDURE dbo.usp_purchases_upsert
...
BEGIN
    ...
    -- 参照発注の存在チェック
    IF @purchase_order_id IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM purchase_orders WHERE purchase_order_id = @purchase_order_id AND is_deleted = 0
    )
    BEGIN
        RAISERROR(N'参照発注が存在しないか削除済みです。', 16, 1);
        RETURN;
    END
